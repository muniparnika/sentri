 ...
 *
 * Node colours:
 *   🟢 green fill   — page has ≥1 approved test
 *   ⚫ gray fill    — crawled, no tests yet
...
function StaticGraph({ pages, activePage, onNodeClick }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", display: "block" }}>
      ...
      {/* Nodes */}
      {nodes.map((n, i) => {
        const { fill, stroke, strokeW } = nodeColor(n);
...
function D3Graph({ pages, activePage, onNodeClick, d3 }) {
  ...
  useEffect(() => {
    ...
    node.append("circle")
      ...
      .attr("stroke-width", d => nodeColor(d).strokeW);
    ...
    return () => { sim.stop(); };
  }, [pages, activePage, d3, onNodeClick]);
...
export default function SiteGraph({ pages = [], activePage, onNodeClick, isRunning }) {
    ...
    <div>
      ...
      <div style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden", minHeight: 200 }}>
        {pages.length === 0 ? (
          ...
          <D3Graph pages={pages} activePage={activePage} onNodeClick={onNodeClick} d3={d3} />
        ) : (
          <StaticGraph pages={pages} activePage={activePage} onNodeClick={onNodeClick} />
        )}
      </div>
    </div>
  );
