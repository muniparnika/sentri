...
- **Cross-side constants**: Event-type literals and enum strings referenced by both backend and frontend live under `backend/src/constants/` (not a repo-root `shared/` directory — the backend Docker image only copies `backend/`, so a sibling top-level `shared/` would break the container build). The frontend reaches into `backend/` via `server.fs.allow: ['..']` in `frontend/vite.config.js` and imports relatively (`../../../backend/src/constants/<thing>.js`). See `backend/src/constants/activityTypes.js` for the canonical example — the `ACTIVITY_TYPES` enum was extracted after a `"test.approve"` vs `"test.approved"` typo regression.
