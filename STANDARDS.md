...
- **Shared modules (`shared/` at repo root)**: Cross-side constants (event-type literals, enum strings) that both backend and frontend reference belong in `shared/<thing>.js` and are imported relatively from both sides. Vite is already configured to serve from the repo root via `server.fs.allow` (see `frontend/vite.config.js`). See `shared/activityTypes.js` for the canonical example — the `ACTIVITY_TYPES` enum was extracted after a `"test.approve"` vs `"test.approved"` typo regression.
